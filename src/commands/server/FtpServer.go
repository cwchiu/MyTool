...
// GetSettings 取得配置
func (driver MainDriver) GetSettings() (*ftpserver.Settings, error) {
	var config ftpserver.Settings
	// config.MaxConnections = 10
	config.ListenAddr = fmt.Sprintf("0.0.0.0:%d", driver.port)
...
func SetupFtpCommand(rootCmd *cobra.Command) {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			driver := NewMyDriver(port, username, password)
			fserver := ftpserver.NewFtpServer(driver)
