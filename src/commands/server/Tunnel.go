...
func SetupTunnelCommand(rootCmd *cobra.Command) {
	var to_port int
	var to_host string
	...
	cmd := &cobra.Command{
		Use:   "tunnel",
		Short: "localtunnel service",
		Run: func(cmd *cobra.Command, args []string) {
			c := NewClient("https://localtunnel.me")
			t := c.NewTunnel(to_host, to_port)
			err := t.OpenAs(subdomain)
			// err := t.Open()
			if err != nil {
				panic(err)
			}
			remote_url := t.URL()
			fmt.Printf("%s:%d => %s", to_host, to_port, remote_url)
			...
			if open_browser {
				browser.OpenURL(remote_url)
			}

			<-t.Closing()
		},
	}
	cmd.Flags().StringVarP(&subdomain, "subdomain", "d", "", "子網域")
	cmd.Flags().IntVarP(&to_port, "to-port", "p", 80, "目的通訊埠")
	cmd.Flags().StringVarP(&to_host, "to-host", "t", "127.0.0.1", "目的主機")
